...
async fn map_ok_step_async_transforms() {
    let step = OkMapStepAsync::new(|x: i32| Box::pin(async move { x * 2 }));
    assert_eq!(step.apply_ok_async(21).await, 42);
}

#[tokio::test]
async fn tap_ok_step_async_works() {
    let step = OkTapStepAsync::new(|x: &str| Box::pin(async move { format!("Hello, {x}!") }));
...
async fn inspect_ok_step_async_side_effect_only() {
    ...
    let step = OkInspectStepAsync::new(|_: &i32| {
        Box::pin(async move {
...
async fn if_ok_step_async_branches() {
    let even_flow = OkMapStepAsync::new(|x: i32| Box::pin(async move { x / 2 }));
    let odd_flow = OkMapStepAsync::new(|x: i32| Box::pin(async move { x * 3 + 1 }));
